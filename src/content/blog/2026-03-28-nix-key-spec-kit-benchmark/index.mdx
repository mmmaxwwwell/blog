...
- **[Kicking it off](#kicking-it-off)**: Started the parallel runner, immediately hit a broken dependency parser that scheduled tasks before their prerequisites existed. Fixed it, runner is working, Part 2 covers the results.
...
The connection errors were a symptom. The parser was the bug. Once I fixed it (continuation line carry-forward, task-ID-to-phase resolution, stopping at `###` headings), phases started executing in the right order and agents stopped dying in loops. The parallel runner is a placeholder I'm planning to replace with a proper workflow engine, and this is exactly why. Parsing markdown with regex, hand-rolling backoff in a main loop, managing state across process boundaries with files on disk: these are all solved problems. But for tonight, it works. Part 2 covers what I found in the morning.
